            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ministry') ? 'is-invalid' : '' }}" name="ministry_id" id="ministry_id">
                    @foreach($ministries as $id => $ministry)
                        <option value="{{ $id }}" {{ (old('ministry_id') ? old('ministry_id') : $directoryContent->ministry->id ?? '') == $id ? 'selected' : '' }}>{{ $ministry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tags)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $directoryContent->tags->contains($id)) ? 'selected' : '' }}>{{ $tags }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
