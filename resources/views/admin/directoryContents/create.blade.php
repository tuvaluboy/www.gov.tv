    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.directory-contents.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('detailinformation') ? 'is-invalid' : '' }}" type="text" name="detailinformation" id="detailinformation" value="{{ old('detailinformation', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('directorysubcategory') ? 'is-invalid' : '' }}" name="directorysubcategory_id" id="directorysubcategory_id">
                    @foreach($directorysubcategories as $id => $directorysubcategory)
                        <option value="{{ $id }}" {{ old('directorysubcategory_id') == $id ? 'selected' : '' }}>{{ $directorysubcategory }}</option>
                    @endforeach
                </select>
