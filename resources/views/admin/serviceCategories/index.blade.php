    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.service-categories.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.serviceCategory.title_singular') }}
            </a>
        </div>
    </div>
@endcan
                        ...
                        <tr data-entry-id="{{ $serviceCategory->id }}">
                            ...
                            <td>
                                @can('service_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.service-categories.show', $serviceCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('service_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.service-categories.edit', $serviceCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('service_category_delete')
                                    <form action="{{ route('admin.service-categories.destroy', $serviceCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-ServiceCategory:not(.ajaxTable)').DataTable({ buttons: dtButtons })
