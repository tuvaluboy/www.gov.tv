...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.ministryContent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ministry-contents.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label for="files">{{ trans('cruds.ministryContent.fields.files') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.ministryContent.fields.files_helper') }}</span>
            </div>
            <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.ministry-contents.storeCKEditorImages') }}', true);
...
Dropzone.options.filesDropzone = {
    url: '{{ route('admin.ministry-contents.storeMedia') }}',
    maxFilesize: 600, // MB
    addRemoveLinks: true,
    headers: {
    ...
    init: function () {
@if(isset($ministryContent) && $ministryContent->files)
          var files =
            {!! json_encode($ministryContent->files) !!}
              for (var i in files) {
              var file = files[i]
