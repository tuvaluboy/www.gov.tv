...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.servicesSubCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.services-sub-categories.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('servicescategory') ? 'is-invalid' : '' }}" name="servicescategory_id" id="servicescategory_id">
                    @foreach($servicescategories as $id => $servicescategory)
                        <option value="{{ $id }}" {{ old('servicescategory_id') == $id ? 'selected' : '' }}>{{ $servicescategory }}</option>
                    @endforeach
                </select>
